...
describe('useEmails', () => {
  ...
  it('marks email as read', async () => {
    ...
    expect(updateEmail).toHaveBeenCalledWith('test-user-id', '1', { read: true });
  ...
  it('marks email as unread', async () => {
    ...
    expect(updateEmail).toHaveBeenCalledWith('test-user-id', '2', { read: false });
    expect(emailCache.invalidateEmails).toHaveBeenCalledWith('test-user-id');
  ...
  it('updates email priority', async () => {
    ...
    expect(updateEmail).toHaveBeenCalledWith('test-user-id', '1', { priority: 'low' });
    expect(emailCache.invalidateEmails).toHaveBeenCalledWith('test-user-id');
  ...
  it('syncs emails', async () => {
    ...
    vi.mocked(emailSyncService.syncEmails).mockResolvedValue({ success: true, errors: [] });

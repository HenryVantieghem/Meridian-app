...
export function useSlackMessages(
  options: UseSlackMessagesOptions = {},
  ...
  const fetchMessages = useCallback(
    async (pageNum = 1, append = false) => {
      ...
      try {
        ...
        const cachedMessages = await slackCache.getMessages(
          workspaceId || "all",
          channelId,
        );
      ...
      const workspaceList =
        integrations
          ?.map((integration) => ({
            ...
            name:
              integration.provider_data?.workspace_name || "Unknown Workspace",
  ...
  const fetchChannels = useCallback(async () => {
    ...
    try {
      ...
      if (data.channels) {
        setChannels(
          data.channels.map((channel: unknown) => ({
  ...
  const sendMessage = useCallback(
    async (text: string, targetChannelId?: string) => {
      if (!userId || !workspaceId) return;

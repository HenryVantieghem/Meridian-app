...
export function useEmails(options: UseEmailsOptions = {}): UseEmailsReturn {
  ...
  const markAsRead = useCallback(
    async (emailId: string) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to mark email as read",
        );
      }
    },
    [userId],
  ...
  const markAsUnread = useCallback(
    async (emailId: string) => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to mark email as unread",
        );
      }
    },
    [userId],
  ...
  const updatePriority = useCallback(
    async (emailId: string, priority: EmailPriority) => {
      if (!userId) return;
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to update email priority",
        );
      }
    },
    [userId],

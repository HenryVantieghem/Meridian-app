...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate variations
    const variations = await generateVariations(systemPrompt, userPrompt, mainDraft);

    // Generate subject if not provided
    const generatedSubject = subject || await generateSubject(context, mainDraft);
...
function buildSystemPrompt(tone: string, context: string, priority: string): string {
  ...
  const toneInstructions = {
    executive: 'Write in a decisive, strategic tone. Be authoritative but not arrogant. Focus on outcomes and decisions.',
    diplomatic: 'Use tactful language that builds relationships. Be respectful and collaborative while maintaining professionalism.',
    direct: 'Be clear, concise, and to the point. No unnecessary words. Focus on actionable information.',
    warm: 'Use a friendly, approachable tone while maintaining professionalism. Show genuine interest in the recipient.',
    formal: 'Use traditional business language. Be ceremonial and structured in your approach.',
    urgent: 'Convey appropriate urgency without being panicked. Focus on time-sensitive actions and clear next steps.'
  };

  const contextInstructions = {
    reply: 'You are responding to a received message. Acknowledge their communication and provide a thoughtful response.',
    'follow-up': 'You are following up on previous communication. Reference prior discussions and move the conversation forward.',
    new: 'You are initiating a new conversation. Provide clear context and purpose for your communication.',
    announcement: 'You are making an announcement. Be clear, comprehensive, and anticipate questions.',
    meeting: 'You are communicating about a meeting. Include relevant details and clear action items.',
    decision: 'You are communicating a decision. Be clear about the decision, rationale, and next steps.'
  };

  const priorityInstructions = {
    high: 'This is high priority. Reflect appropriate urgency and importance in your tone.',
    medium: 'This is standard business priority. Maintain professional urgency.',
...
async function generateVariations(systemPrompt: string, userPrompt: string, mainDraft: string): Promise<string[]> {
  try {
    const variations = [];
...
async function generateSubject(context: string, content: string): Promise<string> {
  try {
...
Generate a clear, professional subject line that accurately reflects the content and purpose.`;
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4',
      messages: [
        { role: 'system', content: 'You are a professional email subject line generator. Create clear, concise subject lines.' },
...
function calculateConfidence(tone: string, context: string, content: string): number {
  ...
  if (content.includes('Thank you') || content.includes('Best regards')) confidence += 5; // Professional closing

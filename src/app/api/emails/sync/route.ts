  ...
  try {
    ...
    for (const gmailEmail of gmailEmails) {
      try {
        ...
        if (upsertError) {
          errors.push(
            `Failed to store email ${gmailEmail.id}: ${upsertError.message}`,
          );
        } else {
          emailsSynced++;
        }
    ...
    logger.info(
      `Email sync completed for user ${userId}: ${emailsSynced} emails, ${errors.length} errors`,
    );
    ...
    logger.error(
      `Email sync API error for user ${userId}: ${error instanceof Error ? error.message : String(error)}`,
    );
    return NextResponse.json(

...
import { Crown, TrendingUp, TrendingDown, Users, MessageCircle, Clock, Star, AlertTriangle, CheckCircle, BarChart3, Activity } from 'lucide-react';
...
export default function VIPInsightsDashboard({ className = '' }: VIPInsightsDashboardProps) {
  const [vipInsights, setVipInsights] = useState<VIPInsight[]>([]);
  const [selectedTimeframe, setSelectedTimeframe] = useState<'week' | 'month' | 'quarter'>('month');
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'board' | 'investor' | 'client' | 'partner'>('all');
  ...
  const averageNetworkHealth = vipInsights.reduce((sum, insight) => sum + insight.networkHealth, 0) / vipInsights.length || 0;
  const atRiskCount = vipInsights.filter(insight => insight.riskLevel === 'high' || insight.riskLevel === 'medium').length;
  const positiveEngagement = vipInsights.filter(insight => insight.engagementTrend === 'up').length;
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <Typography variant="h4" className={`font-bold ${getNetworkHealthColor(averageNetworkHealth)}`}>
                  {Math.round(averageNetworkHealth)}%
                </Typography>
              </div>
              <div className={`p-3 rounded-lg ${getNetworkHealthBg(averageNetworkHealth)}`}>
                <BarChart3 className={`w-6 h-6 ${getNetworkHealthColor(averageNetworkHealth)}`} />
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <Typography variant="h4" className={`font-bold ${atRiskCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  {atRiskCount}
                </Typography>
              </div>
              <div className={`p-3 rounded-lg ${atRiskCount > 0 ? 'bg-red-100' : 'bg-green-100'}`}>
                <AlertTriangle className={`w-6 h-6 ${atRiskCount > 0 ? 'text-red-600' : 'text-green-600'}`} />
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-brand-burgundy rounded-full flex items-center justify-center text-white font-bold">
                    {insight.vipName.split(' ').map(n => n[0]).join('')}
                  </div>
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge className={`${getCategoryColor(insight.category)} flex items-center gap-1`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div className="text-center">
                  <div className={`text-2xl font-bold ${getNetworkHealthColor(insight.networkHealth)}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Typography variant="h6" className="font-semibold text-black mb-2 flex items-center gap-1">
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span>Last interaction: {insight.lastInteraction.toLocaleDateString()}</span>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button size="sm" className="bg-brand-burgundy text-white hover:bg-brand-burgundy/90">
                    Schedule Follow-up
                  </Button>
                </div>
              </div>

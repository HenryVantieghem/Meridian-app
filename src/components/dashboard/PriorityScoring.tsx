...
export class PriorityScoring {
  ...
  private scoreSender(sender: string): number {
    ...
    // C-suite patterns
    if (
      sender.toLowerCase().includes("ceo") ||
      sender.toLowerCase().includes("founder") ||
      sender.toLowerCase().includes("president")
    )
      return 90;
  ...
  private scoreUrgency(subject: string, body: string): number {
    ...
    if (content.includes("today") || content.includes("this morning"))
      urgencyScore += 25;
    if (content.includes("tomorrow") || content.includes("next week"))
      urgencyScore += 10;
  ...
  private scoreVIP(contact: string): number {
    ...
    const vipMatch = this.vipContacts.find(
      (vip) =>
  ...
  private scoreEngagement(contact: string): number {
    ...
    const engagementHistory = this.behaviorData[contact] || {
      replies: 0,
      opens: 0,
      time: 0,
    };
    ...
    if (engagementHistory.time < 3600)
      score += 15; // Less than 1 hour
  ...
  private generateExplanation(factors: any): string {
    ...
    if (explanations.length === 0)
      explanations.push("Standard priority assessment");
...
export function usePriorityScoring() {
  ...
  return {
    scoreItem,
    updateVIPContacts: (contacts: VIPContact[]) =>
      priorityEngine.updateVIPContacts(contacts),
    updateBehaviorData: (contact: string, data: any) =>
      priorityEngine.updateBehaviorData(contact, data),

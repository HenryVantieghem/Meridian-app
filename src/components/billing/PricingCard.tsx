...
export const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const monthlyPrice = tier === 'FREE' ? 0 : isYearly ? price.amount / 12 : price.amount;
  const savings = isYearly && tier !== 'FREE' ? Math.round((price.amount * 12 - price.amount) / 100) : 0;
        ...
        <div className="p-6">
          ...
          <div className="text-center mb-6">
            <div className="flex items-center justify-center mb-4">
              <div className={cn(
                ...
                <Icon className={cn("w-6 h-6", tier === 'PRO' ? "text-black" : config.color)} />
              </div>
            </div>
          ...
          {tier !== 'FREE' && (
            <div className="mb-6 p-4 bg-white/50 rounded-lg">
              <h4 className="text-sm font-semibold text-gray-900 mb-2">Included Limits</h4>
              <div className="space-y-2 text-xs text-gray-600">
                <div className="flex justify-between">
                  <span>Emails per month</span>
                  <span className="font-medium">
                    {product.limits.emailsPerMonth === -1 ? 'Unlimited' : product.limits.emailsPerMonth.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>AI analyses</span>
                  <span className="font-medium">
                    {product.limits.aiAnalyses === -1 ? 'Unlimited' : product.limits.aiAnalyses.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Storage</span>

...
export const emailSchemas: {
  ...
  emailAnalysis: z.object({
    ...
    suggestedActions: z
      .array(z.enum(["reply", "delegate", "archive", "schedule", "forward"]))
      .max(5),
    keyPoints: z.array(z.string().min(1).max(200)).max(10),
    tags: z.array(z.string().min(1).max(50)).max(10).optional(),
    category: z
      .enum(["work", "personal", "spam", "newsletter", "notification"])
      .optional(),
  }),
...
export const userSchemas = {
  ...
  userSettings: z.object({
    ...
    blockedDomains: z
      .array(z.string().regex(/^[a-zA-Z0-9\-\.]+$/, "Invalid domain format"))
      .max(50)
      .optional(),
  }),
};

...
class RealtimeServer {
  ...
  private handleSubscribe(userId: string, channels: string[]) {
    ...
    if (client) {
      client.subscriptions = [
        ...new Set([...client.subscriptions, ...channels]),
      ];
    ...
    updates.forEach((update) => {
      this.clients.forEach((client, userId) => {
        if (
          ...
          client.subscriptions.includes(update.type)
        ) {
          this.sendToUser(userId, {
            type: "update",
            data: update,
  ...
  public sendUpdateToUser(
    userId: string,
    update: Omit<RealtimeUpdate, "id" | "timestamp">,
  ) {
    const fullUpdate: RealtimeUpdate = {
      ...update,
      id: Math.random().toString(36).substr(2, 9),
...
export function sendUpdateToUser(
  userId: string,
  update: Omit<RealtimeUpdate, "id" | "timestamp">,
) {
  const server = getRealtimeServer();

...
export class DatabaseBackupManager {
  ...
  async cleanupOldBackups(): Promise<void> {
    ...
    retentionDate.setDate(
      retentionDate.getDate() - BACKUP_CONFIG.DATABASE.RETENTION_DAYS,
    );
...
export class UserDataExportManager {
  ...
  async exportUserData(
    ...
    try {
      ...
      logger.error("User data export failed", error as Error, {
        userId,
        exportId,
      });
  ...
  private async performUserDataExport(
    userId: string,
    format: "json" | "csv",
  ...
  private async uploadExportData(
    data: string,
    userId: string,
    format: string,
  ): Promise<string> {

...
// Encryption
export { EncryptionService, encryptionService } from "../security/encryption";
  ...
  async cleanupOldBackups(): Promise<{
    ...
    try {
      ...
      for (const backup of backups) {
        ...
        const ageInDays =
          (now.getTime() - new Date(backup.createdAt).getTime()) /
          (1000 * 60 * 60 * 24);
  ...
  checkBackupHealth: async (): Promise<{
    ...
    if (stats.lastBackup) {
      const daysSinceLastBackup =
        (Date.now() - stats.lastBackup.getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceLastBackup > 7) {
        issues.push(
          `Last backup was ${Math.floor(daysSinceLastBackup)} days ago`,
        );
      }
    } else {

...
export { PerformanceMonitoringService } from "../monitoring/performance-monitoring";
...
export const performanceMonitoring = {
  ...
  trackBundleSize: (bundleStats: unknown) => {
    ...
    if (totalSize > budget) {
      console.warn(
        `Bundle size exceeds budget: ${(totalSize / 1024).toFixed(2)}KB > ${(budget / 1024).toFixed(2)}KB`,
      );
    }
  },
};

      ...
      messages: [
        {
          role: "system",
          content:
            "You are Napoleon AI, an intelligent email assistant. Analyze emails with precision and extract actionable insights. Always respond with valid JSON.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return {
      ...
      actionItems: (analysis.actionItems || []).map(
        (item: unknown, index: number) => ({
          ...
          type: (item as { type?: string }).type || "follow-up",
          title: (item as { title?: string }).title || "",
          description: (item as { description?: string }).description || "",
          priority: (item as { priority?: string }).priority || "medium",
          dueDate: (item as { dueDate?: string }).dueDate,
          assignee: (item as { assignee?: string }).assignee,
          emailId: email.id,
        }),
      ),
...
export async function extractActions(
  analyses: (EmailAnalysis | MessageAnalysis)[],
): Promise<ActionItem[]> {
  try {

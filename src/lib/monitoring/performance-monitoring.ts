...
export class PerformanceMonitoringService {
   ...
   */
  trackDatabaseQuery(query: string, duration: number, requestId: string): void {
    const metric = {
      type: "database_query",
  ...
  private sanitizeQuery(query: string): string {
    return query.replace(/\s+/g, " ").replace(/['"]/g, "?").substring(0, 200);
  }
  ...
  getPerformanceSummary(): {
    ...
    const errorRate =
      this.metrics.filter((m) => m.hasError).length / totalRequests;
